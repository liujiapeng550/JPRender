...
oclLightSourceDistant::oclLightSourceDistant(oclInt id)
  :oclLightSource(id,LIGHT_TYPE::DISTANT,1u){
	m_lightData.lightFuncIndex=CFINDEX_LIGHT_DISTANT;
...
oclLightSourceDistant::~oclLightSourceDistant(){}

oclBool oclLightSourceDistant::Init(){
  Json::Value Property;
...
void oclLightSourceDistant::SetPropertyToData_(const oclChar* propName,VARIANT* v){
...
oclBool oclLightSourceDistant::UpdateWorldMatrix(oclFloat* matrix){
  oclFloat3 vec3;
  vec3.x=matrix[3];
  vec3.y=matrix[7];
  vec3.z=matrix[11];
...
oclBool oclLightSourceDistant::SetCenter(oclFloat* center){
...
oclBool oclLightSourceDistant::NeedUpload() const {
